...
void zigzag_motion_1D(Point3D start, Point3D end,FILE *fptr, FILE *fptr_agnlerad, FILE *fptr_angledegree) {
...
void zigzag_motion_2D(Point3D start, Point3D end,FILE *fptr ,FILE *fptr_agnlerad, FILE *fptr_angledegree) {
    int dir = 1; // 1 = forward in Y, -1 = backward
    int x = start.x;
    float theta_e = 0;
    for (int y = start.y; y <= end.y; y += STEP_Y) {
...
int main() {
    ...
    FILE *fptr_agnlerad;
    fptr_agnlerad = fopen("datafiles/Angles_radians.txt", "w");  
    if (!fptr_agnlerad) {
        perror("Error opening file");
        return 1;
    }
    FILE *fptr_angledegree;
    fptr_angledegree = fopen("datafiles/Angles_degrees.txt", "w");  
    if (!fptr_angledegree) {
        perror("Error opening file");
        return 1;
    }
    ...
    fprintf(fptr_angledegree , "xe   ye   ze theta_0   theta_1   theta_2   theta_3\n");
    ...
    zigzag_motion_2D(p3,p4, fptr,fptr_agnlerad,fptr_angledegree);
    zigzag_motion_1D(p5,p6,fptr,fptr_agnlerad,fptr_angledegree);
